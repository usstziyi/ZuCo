...
def analyze_pkl_v1(data_dir):
    """读取 load_data_v1_rich.py 生成的 ZuCo1.0 pkl，统计每个 pkl 的 subject_name 数量及每个 subject_name 的句子数。"""
    tasks = ['task1-SR', 'task2-NR', 'task3-TSR']

    for task in tasks:
        pkl_path = os.path.join(data_dir, f'{task}_v1.pkl')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='统计 ZuCo1.0 pkl 中每个 subject 的句子数量')
    parser.add_argument('--data_dir', type=str, default='EEG_data/ZuCo1.0', help='ZuCo1.0 pkl 目录')
    args = parser.parse_args()

    analyze_pkl_v1(args.data_dir)